...
public class Configuration {
	/**
	 * The generator can generate also a comments that can help to understand, why
	 * is generated the code in certain way. Default value: false
	 */
	...
	public void load() throws Exception {
		...
		if (globalProperties.get(P_BASE_INTERFACE_PACKAGE).equals(defaultProp.get(P_BASE_INTERFACE_PACKAGE))
				&& !globalProperties.get(P_TARGET_INTERFACE_PACKAGE)
						.equals(defaultProp.get(P_TARGET_INTERFACE_PACKAGE))) {
				...
				if (whitelist != null && whitelist instanceof List whiteList) {
					for (Object item : whiteList) {
						if (item instanceof Map whitelistItem) {
							...
							if (clazz != null
									&& (clazz.equals(classDef.getFullname()) || clazz.equals(classDef.getName()))) {
