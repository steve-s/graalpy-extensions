...
public class TransformerVisitor implements NodeVisitor<String> {
	...
	public String visit(MypyFile mypyFile) {
		...
		// currentContext.addImport(configuration.getBaseAPIPackage(currentContext) +
		// ".GuestValue");
		...
		// System.out.println(printSymbolTableNames(mypyFile.getNames(), "Symbol Table
		// of file " + mypyFile.getName()));
		StringBuilder factoryMethods = new StringBuilder();
		Object generateTypeInterface = configuration.getProperties(currentContext)
				.get(Configuration.P_GENERATE_TYPE_INTERFACE);
	...
	public String visit(ClassDef classDef) {
		if (currentContext.isIgnored(classDef.getFullname()) || classDef.getName().startsWith("<subclass of")) {
		...
		if (!isTupleType) {
			...
			if (!bases.isEmpty()) {
				for (int i = 0; i < bases.size(); i++) {
					...
					// String guestValueFQN = configuration.getBaseAPIPackage(currentContext) +
					// ".GuestValue";
					// TODO make it configurable
					if ("builtins.object".equals(pythonType) || "builtins.type".equals(pythonType)) {
						// currentContext.addImport(guestValueFQN);
						if (bases.size() == 1) {
							extendsExpr.append(getBaseInterface(currentContext));
						}
					} else {
						// extendsExpr.append(currentContext.useType(pythonType,
						// TypeManager.get().resolveJavaType(currentContext, bases.get(i))));
						String defaultExtends = currentContext.getConfiguration()
								.get(Configuration.P_BASE_INTERFACE_PACKAGE) + "."
								+ currentContext.getConfiguration().get(Configuration.P_BASE_INTERFACE_NAME);
						String javaType = TypeManager.get().resolveJavaType(currentContext, bases.get(i), false,
								defaultExtends);
		...
		if (currentContext.getConfig().addLogComments(currentContext)) {
			...
			template = template.replace("{{logcomment}}",
					"{{indent}}// TypeInfo was not found for class " + classDef.getName());
		}
		...
		content.append(TEMPLATE_IS_METHOD.replace("{{python_fqn}}", classDef.getFullname())).append("\n\n");
		...
		try {
			GeneratorUtils.saveFile(currentContext, packageName, interfaceName,
		...
		if (generateTypeInterface.equals(true) || hasStaticMember) {
			...
			template = template.replace("{{javadoc}}", GeneratorUtils
					.wraptToJavadoc("This interface expose static fields and methods of the class.", 0, true));
			...
			if (generateTypeInterface.equals(true) || GeneratorUtils.hasStaticMember(typeInfo)) {
				...
				factoryMethods.append(TEMPLATE_TYPE_FROM_CONTEXT.replace("{{name}}", interfaceName)
						.replace("{{python_fqn_module}}",
								classDef.getFullname().substring(0, classDef.getFullname().lastIndexOf('.')))
			...
			template = template.replace("{{content}}",
					GeneratorUtils.indentTemplate(currentContext, content.toString()));
			template = template.replace("{{factory_methods}}",
					GeneratorUtils.indentTemplate(currentContext, factoryMethods.toString()));
			currentContext.decreaseIndentLevel();
			...
			try {
				GeneratorUtils.saveFile(currentContext, packageName, interfaceTypeName,
						GeneratorUtils.indentTemplate(currentContext, template));
			} catch (IOException ex) {
				Logger.getLogger(TransformerVisitor.class.getName()).log(Level.SEVERE, null, ex);
			}
	...
	public String visit(FuncDef funcDef) {
		...
		for (int i = 0; i < fnSignatureGenerators.length; i++) {
			...
			String text = GeneratorFactory.createFunctionGenerator(fnInterfaceGenerator).createSignature(funcDef,
					currentContext);
	...
	public String visit(Var v) {
		if (currentContext.isClass() || currentContext.isFile()) {
			...
			if (generateFieldGetters) {
				...
				for (int i = 0; i < fnSignatureGenerators.length; i++) {
					String fnInterfaceGenerator = fnSignatureGenerators[i];
					String text = GeneratorFactory.createFunctionGenerator(fnInterfaceGenerator).createSignature(v,
							currentContext);
					sb.append(text).append('\n');
				}
				sb.append("\n");
		...
		// configuration.functionGenerators(currentContext);
		// for (int i = 0; i < fnSignatureGenerators.length; i++) {
		// String fnInterfaceGenerator = fnSignatureGenerators[i];
		// String text =
		// GeneratorFactory.createFunctionGenerator(fnInterfaceGenerator).createSignature(v,
		// currentContext);
		// sb.append(text).append(";\n");
		// }
		// sb.append("\n");
	...
	public String visit(TypeInfo typeInfo) {
		String result;
		// System.out.println(printSymbolTableNames(typeInfo.getNames(), "Symbol Table
		// of TypeInfo " + typeInfo.getFullname()));
		GeneratorContext tmpContext = currentContext;
		...
		if (typeInfo.getTupleType() instanceof TupleType tupleType) {
			...
			// currentContext.getConfig().getProperties(currentContext).get(Configuration.P_TYPE_GENERATOR);
	...
	public String visit(OverloadedFuncDef oFnDef) {
		...
		for (int i = 0; i < fnSignatureGenerators.length; i++) {
			String fnInterfaceGenerator = fnSignatureGenerators[i];
			String text = GeneratorFactory.createFunctionGenerator(fnInterfaceGenerator).createSignature(oFnDef,
					currentContext);
			sb.append(text);
		}
		sb.append("\n");
	...
	public String visit(Decorator decorator) {
		...
		// sb.append("\n{{indent}}// TypeInfo:
		// ").append(decorator.getInfo().toString());
	...
	private String generateFactoryMethod(String interfaceName, String moduleName) {
		...
		String template = "{{indent}}interface {{pname}} {\n"
			...
			public String visit(ClassDef classDef) {
				...
				if (lastInfo != null) {
					...
					if (initMethod != null && initMethod.getNode() instanceof FuncDef fnInit) {
						...
						List<List<OverloadArgsGenerator.Argument>> argVariations = OverloadArgsGenerator
								.generateVariations(context, fnInit);

						for (List<OverloadArgsGenerator.Argument> args : argVariations) {

							if (!args.isEmpty() && args.getLast().getKind() == ArgKind.ARG_STAR2
									&& kwArgsBuilderFQN != null) {
				...
				if (!argsResolved) {
					result.append(template.replace("{{args}}", "").replace("{{arg_names}}", ""));
				}

				result.append(template.replace("{{args}}", "KeywordArguments arguments").replace("{{arg_names}}",
						"arguments"));
