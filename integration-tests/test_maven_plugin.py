...
class MavenPluginTest(util.BuildToolTestBase):
    ...
    def check_generated_app(self, use_default_vfs_path):
        with util.TemporaryTestDirectory() as tmpdir:
            ...
            util.check_ouput("BUILD SUCCESS", out, logger=log)
            util.check_ouput("Deleting GraalPy venv due to changed launcher path", out, logger=log)
            util.check_ouput("hello java", out, logger=log)
            util.check_ouput("not compatible with the current platform", out, contains=False, logger=log)
