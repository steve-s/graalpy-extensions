 ...
 *
 * If there is no virtual environment preset then it is first created and
 * packages are installed the same way as in scope of
 * {@link InstallPackagesTask}.
 */
...
public abstract class LockPackagesTask extends AbstractPackagesTask {
	...
	public void exec() throws GradleException {
		...
		try {
			VFSUtils.lockPackages(venvDirectory, getPackages().get(), getLockFilePath(), LOCK_FILE_HEADER,
					createLauncher(), getPolyglotVersion().get(), getLog());
		} catch (IOException e) {
			throw new GradleException(
					String.format("failed to lock packages in python virtual environment %s", venvDirectory), e);
		}
	}
