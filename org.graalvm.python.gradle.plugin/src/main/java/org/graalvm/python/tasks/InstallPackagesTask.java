	...
	public void exec() throws GradleException {
		...
		try {
			...
			String pluginPkgsString = pce.getPluginPackages().isEmpty()
					? "None"
					: String.join(", ", pce.getPluginPackages());
			String lockFilePkgsString = pce.getLockFilePackages().isEmpty()
					? "None"
					: String.join(", ", pce.getLockFilePackages());
			throw new GradleException(
					String.format(PACKAGES_CHANGED_ERROR, lockFilePath, pluginPkgsString, lockFilePkgsString));
		} catch (IOException e) {
			throw new GradleException(String.format("failed to create python virtual environment in %s", venvDirectory),
					e);
		}
	}
}
