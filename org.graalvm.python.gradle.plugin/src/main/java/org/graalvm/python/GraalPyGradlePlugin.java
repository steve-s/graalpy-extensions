...
public abstract class GraalPyGradlePlugin implements Plugin<Project> {
	...
	public void apply(Project project) {
		...
		TaskProvider<InstallPackagesTask> installPackagesTask = registerInstallPackagesTask(project, launcherClasspath,
				extension);
		...
		project.afterEvaluate(proj -> {
			...
			if (extension.getPythonResourcesDirectory().isPresent()) {
				proj.getLogger().warn(
						"WARNING: Property 'pythonResourcesDirectory' is deprecated and will be removed. Use property 'externalDirectory' instead.");
			}

			// Run the vfsFilesListTask conditionally only if 'externalDirectory' is not set
			if (!extension.getPythonResourcesDirectory().isPresent() && !extension.getExternalDirectory().isPresent()) {
				if (!extension.getResourceDirectory().isPresent()) {
					proj.getLogger().warn(String.format(
							"Virtual filesystem is deployed to default resources directory '%s'. "
	...
	private void registerMetaInfTask(GraalPyExtension extension) {
		var metaInfTask = project.getTasks().register(GRAALPY_META_INF_TASK_TASK, MetaInfTask.class, t -> {
			t.getResourceDirectory().set(extension.getResourceDirectory());
			t.getManifestOutputDir()
					.convention(project.getLayout().getBuildDirectory().dir(GRAALPY_META_INF_DIRECTORY));
			t.setGroup(GRAALPY_GRADLE_PLUGIN_TASK_GROUP);
		});
	 ...
	 *            the classpath of the Python launcher
	 * @return the resources task provider
	 */
	private TaskProvider<InstallPackagesTask> registerInstallPackagesTask(Project project,
			Configuration launcherClasspath, GraalPyExtension extension) {
	...
	private void registerPackagesTask(Project project, Configuration launcherClasspath, GraalPyExtension extension,
			AbstractPackagesTask t) {
		...
		RegularFile graalPyLockFile = extension.getGraalPyLockFile().orElse(projectDirectory.file(GRAALPY_LOCK_FILE))
				.get();
	...
	/**
	 * Adds implicit dependencies to the project based on the extension
	 * configuration.
	 */
	private void addDependencies() {
		...
		implementation.getDependencies()
				.addAllLater(dependencyList(project, extension,
						(version, community) -> List.of(dependency(POLYGLOT_GROUP_ID, POLYGLOT_ARTIFACT_ID, version),
	...
	private static void makeSureBothEditionsAreNotOnClasspathSimultaneously(ConfigurationContainer configurations) {
		configurations.getByName(JavaPlugin.RUNTIME_CLASSPATH_CONFIGURATION_NAME).getIncoming()
	...
	private Provider<? extends Iterable<Dependency>> dependencyList(Project project, GraalPyExtension extension,
			BiFunction<String, Boolean, List<String>> generator) {
	...
	public static String determineGraalPyDefaultVersion() {
		...
		if (version == null) {
			try {
				InputStream propertiesStream = GraalPyGradlePlugin.class.getClassLoader()
						.getResourceAsStream(GRADLE_PLUGIN_EXTRA_PROPERTIES);
				...
				throw new IllegalStateException(
						String.format("Failed to read the GraalPy version from '%s'.", GRADLE_PLUGIN_EXTRA_PROPERTIES),
						e);
			}
		}
		return version;
